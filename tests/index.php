...
function py_faker_desc($desc, $code)
{
	$faker = Factory::create('zh_CN');
	ob_start();
	eval("\$result = $code;");
	...
	echo "<p style='border-bottom: 1px solid #EFEFEF;line-height: 1.4;font-size: 13px;padding-bottom: 8px;'>
	{$desc} : {$codeHl} <br/> {$content}
</p>";

}

function py_faker_title($title)
{
...
foreach ($base as $item) {
	py_faker_desc($item[0], (string) $item[1]);
}

py_faker_title('Lorem');
...
py_faker_desc('随机生成一个唯一字串', '$faker->uuid');

py_faker_title('Calculator');
